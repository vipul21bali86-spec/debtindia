...
function formatRupees(value) { return '₹' + INR.format(Math.floor(value)); }
function formatLakhCrore(value) { return '₹' + value.toFixed(2).replace(/\.00$/, '') + ' lakh cr'; }
function formatDate(dateString) { return new Date(dateString).toLocaleString('en-IN', { dateStyle: 'medium', timeStyle: 'short' }); }
function ratio(a, b) { return (a / b).toFixed(1) + 'x'; }
function setWidth(id, part, total) { document.getElementById(id).style.width = `${Math.max(4, (part / total) * 100)}%`; }
...
function renderCountries(countries) {
  ...
  grid.innerHTML = countries.map(country => `
    <div class="country-card ${country.highlight ? 'highlight' : ''}">
      <div class="country-name">${country.name}</div>
      <div class="country-debt">${country.debtToGdp}%</div>
      <div class="small-copy">Debt / GDP</div>
      <div class="note" style="margin-top:8px;">${country.takeaway}</div>
    </div>
  `).join('');
}
...
async function loadDebt() {
  ...
  const debtRatePerSecond = (target - base) / ((targetMs - baseMs) / 1000);
  ...
  document.getElementById('headlineSubcopy').textContent = `${formatLakhCrore(data.interestPaymentsLakhCrore)} in annual interest. ${data.interestShareOfExpenditure}% of spending is going to past borrowing.`;
  ...
  renderCountries(data.comparisonCountries);

  function render() {
    const now = Date.now();
    const liveDebt = base + ((now - baseMs) / 1000) * debtRatePerSecond;
    document.getElementById('debtValue').textContent = formatRupees(liveDebt);
    document.getElementById('perCitizen').textContent = formatRupees(liveDebt / data.population);
  }
